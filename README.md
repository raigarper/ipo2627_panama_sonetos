...
El cuerpo aporta el esqueleto semántico —`<header>`, `<main>`, `<aside>`, `<article>`, `<footer>`— y los
**puntos de anclaje** que el JavaScript localiza mediante atributos `data-*` (`data-buscador`, `data-soneto`),
nunca por `id` ni por clase de estilo, para que renombrar una clase CSS no rompa el script.
...
Composición estructural. La página es un **Grid** de tres filas (cabecera / lector / pie) y el lector, un Grid
de dos columnas (`minmax(16rem, 26%) 1fr`) que colapsa a una sola bajo `48em`. Los contenedores intermedios
—buscador, soneto, encabezado— son **Flexbox** en columna con `gap`.
...
Bloques: `.cabecera`, `.pie`, `.buscador` (con `__campo`, `__lista`, `__opcion`, `__marca`, `__vacio`) y
`.soneto` (con `__encabezado`, `__titulo`, `__autor`, `__estrofa`, `__verso`, `__mensaje`).
...
- `buscar(consulta)` — trocea la consulta en términos normalizados y puntúa cada soneto: **título ×3,
  ...
  sus campos; la lista se ordena por relevancia y se recorta a `MAXIMO_RESULTADOS` (6). Emite `resultados`.
...
De cada soneto que coincide se guarda además el primer verso donde aparece un término, que el buscador muestra
como vista previa.
...
- `renderizarResultados()` construye cada opción con `createElement` —nunca `innerHTML` con datos— y
  `resaltar()` envuelve en `<mark>` los fragmentos coincidentes buscando sobre el texto normalizado y cortando
  sobre el original, de modo que el resaltado funcione aunque el usuario escriba sin tildes.
...
Pinta el lienzo. `renderizar(soneto)` construye `<header>` + una `<section>` por estrofa + un `<p>` por verso,
...
El cableado, en los dos sentidos: las intenciones del usuario (`alEscribir`, `alAbrir`, `alSeleccionar`) van a
métodos del modelo, y los eventos del modelo (`resultados`, `seleccion`) vuelven como órdenes de pintado.
Es el único punto del programa donde ambas vistas coinciden.
...
vista. No hay estado duplicado; el modelo es la única fuente de verdad.
...
**Accesibilidad.** Contraste por encima de AA en todos los pares de color, foco visible, navegación completa
por teclado, etiquetas asociadas, roles ARIA en el combobox y región `aria-live` en el lienzo.
